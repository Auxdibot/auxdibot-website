...
export async function generateMetadata({ params }: CardProps): Promise<Metadata> {
    const leaderboardData = await axios.get<LeaderboardPayload | { error: string } | undefined>(`${process.env.NEXT_PUBLIC_SITE_URL}/bot/v1/leaderboard/${params.serverID}`, {
        headers: {
    ...
    if (!leaderboardData) return {
        ...
        description: "Couldn't find an Auxdibot leaderboard for that server.",
        openGraph: {
    ...
    return {
        ...
        description: "The leaderboard data for " + leaderboardData.server.name + ". View the top users and their levels, XP, and more. Powered by Auxdibot.",
        icons: leaderboardData.server.icon_url,
        openGraph: {
            title: `${leaderboardData.server.name}`,
            description: "The leaderboard data for " + leaderboardData.server.name + ". View the top users and their levels, XP, and more. Powered by Auxdibot.",
            images: leaderboardData.server.icon_url && [leaderboardData.server.icon_url],
