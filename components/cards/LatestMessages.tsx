...
export default function LatestMessages({ data }: { readonly data: CardData; }) {
    ...
    return <div className={"flex justify-center items-center max-w-sm"}>
    <section className={`${data?.dark ? "bg-black" : "bg-gray-100"} ${data?.dark ? "text-gray-100" : "text-gray-900"} ${data?.dark ? "border-gray-800" : "border-gray-300"} border max-w-full px-3 py-10 bg-opacity-60 rounded-2xl`}>
