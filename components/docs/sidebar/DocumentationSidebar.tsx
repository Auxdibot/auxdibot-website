...
export function DocumentationSidebar({ doc: docPage, docList }: { doc: string | string[] | undefined; docList: ({ id: string; name: string; } | undefined)[];}) {
    ...
    <nav className={`flex fixed w-80 max-md:w-60 h-full bg-gray-950 border-r-2 border-gray-800`}>
        <ScrollArea className=" overflow-y-auto  overflow-visible border-b-0 pb-20 pr-1">
            ...
            {docList?.map((document) => document && <SidebarCategory docID={document.id}  key={document.id} selected={docPage?.toString() ?? "home"} title={document.name} />)}
            </div>
        </ScrollArea>
