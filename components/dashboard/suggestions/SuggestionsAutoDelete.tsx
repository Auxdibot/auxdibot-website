...
export default function SuggestionsAutoDelete({ server }: { server:  { serverID: string, suggestions_auto_delete: boolean } }) {
    ...
    function handleClick() {
        fetch(`/bot/v1/servers/${server.serverID}/suggestions/auto_delete`, { method: "POST" }).then(async (data) => 
        {
            ...
            toast({
                ...
                description: json ? json['suggestions_auto_delete'] ? 'Suggestions will now automatically be deleted when they are responded to.' : 'Suggestions will no longer be automatically deleted.' : 'An error occurred. Please try again.',
