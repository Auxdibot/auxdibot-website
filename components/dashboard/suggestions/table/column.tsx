...
export const columns: (serverID: string) => ColumnDef<SuggestionColumn>[] = (serverID) => [
  {
    ...
    cell({ row }) {
      ...
      function deleteSuggestion() {
        ...
        fetch(`/api/v1/servers/${serverID}/suggestions`, { method: "DELETE", body }).then(async (data) => {
            ...
            toast({ title: "Suggestion Deleted", description: `Suggestion #${row.original.suggestionID} was deleted.`, status: "success" })
        })
        }
        ...
        return (
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className={'max-w-xs'} align="end">
              ...
              <DropdownMenuItem onClick={() => setOpenSuggestion(true)} className={"flex items-center gap-1 transition-all cursor-pointer"}>
                <BsEye/> View Suggestion
              </DropdownMenuItem>
              {row.original.handlerID && <DropdownMenuItem onClick={() => setHandledOpen(true)} className={"flex items-center gap-1 transition-all cursor-pointer"}>
          ...
          <Dialog open={suggestionOpen} onOpenChange={(e) => setOpenSuggestion(e)}>

            <DialogContent>
              <h2 className={"text-xl font-montserrat flex items-center gap-2"}><BsEye/> View Suggestion</h2>
              <DialogDescription>{row.original.content}</DialogDescription>
            </DialogContent>
          </Dialog>
          {row.original.handlerID && <Dialog open={handledOpen} onOpenChange={(e) => setHandledOpen(e)}>

            <DialogContent>
              <h2 className={"text-xl font-montserrat flex items-center gap-2"}><BsHammer/> View Handler & Reason</h2>
              <User userID={row.original.handlerID} />
              <DialogDescription>{row.original.handled_content ?? "No reason given."}</DialogDescription>
            </DialogContent>
          </Dialog>}
        </>)
      }
