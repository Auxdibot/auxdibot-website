...
export default function SuggestionsUpdateChannel({ server }: { server: { id: string }}) {
    ...
    function onSuggestionsUpdateChannelChange(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    function setSuggestionsUpdateChannel() {
        ...
        fetch(`/api/v1/servers/${server.id}/suggestions/updates_channel`, { method: "POST", body }).then(() => {
    ...
    <span className={"flex flex-row max-md:flex-col gap-2"}>
        <select onChange={(e) => onSuggestionsUpdateChannelChange(e)} value={channel} className={"font-roboto w-fit mx-auto rounded-md p-1 text-md"}>
