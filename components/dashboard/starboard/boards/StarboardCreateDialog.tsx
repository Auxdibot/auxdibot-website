...
export function StarboardCreateDialog({
    id,
    onChange,
}: {
    readonly id: string;
    ...
    function onSubmit(data: StarboardCreateBody) {
        ...
        fetch(`/bot/v1/servers/${id}/starboard/boards`, {
            method: 'POST',
            body,
        })
            .then(async (res) => {
                ...
                if (!json || json['error']) {
                    toast({
                        title: `Failed to create new starboard`,
                        description: json['error']
                            ? json['error']
                            : `An error occurred while creating the starboard.`,
                        status: 'error',
                    });
                    return;
                }
                toast({
                    ...
                    description: `The starboard ${data.board_name} was created.`,
                    status: 'success',
                });
                reset();
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button variant={'ghost'} className='group h-9 w-9 p-0'>
                    <Plus className='w-8 text-gray-600 transition-all group-hover:text-gray-200' />
                </Button>
            </DialogTrigger>
            <DialogContent>
