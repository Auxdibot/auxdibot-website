...
export default function StarboardBoards({
    server,
}: {
    server: {
        data: {
    ...
    const [board, setBoard] = useState<string | undefined>(
        server?.data?.starboard_boards[0]?.board_name
    );
    const boardData = useMemo(
        () =>
            server?.data?.starboard_boards?.find((b) => b.board_name == board),
        [board, server]
    );
    ...
    function deleteBoard() {
        ...
        fetch(`/bot/v1/servers/${server.data.serverID}/starboard/boards`, {
            method: 'DELETE',
            body: new URLSearchParams({ board_name: board }),
        })
            .then(async (data) => {
                ...
                toast({
                    title: 'Board Deleted',
                    description: `The starboard board ${board} has been deleted.`,
                    status: 'success',
                });
        ...
        <>
            ...
            >
                ...
                <div className={'flex flex-col gap-4'}>
                    {server?.data && (
                        <span className='my-2 flex gap-2 px-5'>
                            {board && (
                                <AlertDialog>
                            ...
                            >
                                ...
                                <SelectContent>
                                    {server.data.starboard_boards.map(
                                        (b, index) => {
                                            ...
                                            return (
                                                <SelectItem
                                                    ...
                                                    key={index}
                                                >
                                                    <span className='flex items-center gap-2 font-open-sans'>
                                                        <Twemoji
                                                            className='w-4'
                                                            serverID={
                                                                server.data
                                                        ...
                                                        >
                                                            {emojiValue?.hexcode?.toLowerCase() ??
                                                                b.reaction}
                                                        </Twemoji>{' '}
                                                        {b.board_name}
                                                    </span>
