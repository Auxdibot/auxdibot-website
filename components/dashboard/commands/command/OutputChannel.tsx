...
export default function OutputChannel({ command, subcommand, output_channel, id }: { command: string, subcommand?: string[], output_channel?: string, id: string }) {
    ...
    return <span className={"flex-1 min-w-[170px] w-fit max-md:mx-auto"}><Channels value={output_channel ?? ''} serverID={id} onChange={(e) => setOutputChannnel(e.channel)}/> </span>
}