...
export default function DisabledSlider({ command, subcommand, disabled, id }: { command: string, subcommand?: string[], disabled?: boolean, id: string }) {
    ...
    return (<div className={"w-16 h-8 border border-gray-700 rounded-full relative px-1 text-xl"} ><div onClick={() => handleClick()} className={`cursor-pointer absolute rounded-full top-1/2 bottom-1/2 -translate-y-1/2 h-7 w-7 transition-all bg-gradient-to-l flex items-center justify-center ${disabled ? "from-red-500 to-red-700 -translate-x-0.5" : "translate-x-full from-green-500 to-green-700"}`}><span className={"text-white text-md opacity-60"}>{disabled ? <BsX/> : <BsCheck/>}</span></div></div>)
}