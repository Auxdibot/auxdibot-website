...
export default function CommandChannel({ server }: { server: { serverID: string, commands_channel?: string }}) {
    ...
    return <span className={"flex flex-row max-md:flex-col w-fit gap-2"}>
    <span className={"flex-1 max-md:mx-auto"}><Channels value={channel} serverID={server.serverID} onChange={(e) => onCommandChannelChange(e)}/> </span>
    <Button onClick={() => setCommandChannel()} variant={'outline'} className={"flex gap-1 items-center w-fit max-md:mx-auto"} type="submit">
        {success ? (<><BsCheckLg/> Updated!</>) : (<><BsSlash/> Set</>) }
    </Button></span>;
}