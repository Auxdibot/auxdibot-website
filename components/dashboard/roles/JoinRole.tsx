...
export default function JoinRole({ roleID, serverID, index }: { readonly roleID: string, readonly serverID: string; readonly index: number; }) {
    ...
    function deleteException() {
        ...
        fetch(`/api/v1/servers/${serverID}/join_roles/${roleID}`, { method: "DELETE", body }).then(async (data) => {
            ...
            if (actionContext)
                json && json['error'] ? actionContext.setAction({ status: `An error occurred. Error: ${json['error'] || "Couldn't find error."}`, success: false }) : json ? actionContext.setAction({ status: `Successfully deleted join role #${index+1}`, success: true }) : ""
        })
    }
