...
import { BsCheckLg, BsMicMute, BsPersonAdd, BsPersonDown } from "react-icons/bs";
...
export default function Massrole({ server }: { server: { id: string}}) {
    ...
    function setMassrole(data: MassroleBody) {
        ...
        fetch(`/api/v1/servers/${server.id}/massrole`, { method: "POST", body }).then(() => {
            setSuccess(true)
            if (actionContext)
