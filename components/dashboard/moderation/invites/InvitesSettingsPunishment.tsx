...
export default function InvitesSettingsPunishment({ server }: { server: { readonly serverID: string, readonly automod_invites_punishment: AutomodPunishment; }}) {
    ...
    function onSubmit(data: InvitesLimitPunishmentBody) {
        ...
        fetch(`/api/v1/servers/${server.serverID}/moderation/invites/punishment`, { method: 'POST', body }).then(async (data) => {
            const json = await data.json().catch(() => actionContext ? actionContext.setAction({ status: "error receiving data!", success: false }) : {});
            if (json && !json['error']) {
                queryClient.invalidateQueries(['data_moderation', server.serverID]);
                setSuccess(true);
                if (actionContext) 
                    actionContext.setAction({ status: `Successfully updated the invites limit punishment for this server.`, success: true })
    ...
    <form onSubmit={handleSubmit(onSubmit)} className={"flex flex-col items-center justify-center gap-4"}>
        ...
        {success ? (<><BsCheckLg/> Updated!</>) : (<><BsHammer/> Change Invites Punishment</>) }
        </button>
    </form>
    </section>;
}