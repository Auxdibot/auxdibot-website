...
type InvitesLimitPunishmentBody = {
    punishment: PunishmentType | null;
    reason?: string;
};

export default function InvitesSettingsPunishment({
    server,
}: {
    server: {
    ...
    const { control, reset, handleSubmit } =
        useForm<InvitesLimitPunishmentBody>({
            defaultValues: useMemo(
                () => ({
                    punishment:
                        server.automod_invites_punishment?.punishment ?? 'WARN',
                    reason:
                        server.automod_invites_punishment?.reason ??
                        'You have been punished for breaking the invites limit on this server.',
                }),
    ...
    function onSubmit(data: InvitesLimitPunishmentBody) {
        ...
        fetch(
            `/bot/v1/servers/${server.serverID}/moderation/invites/punishment`,
            { method: 'POST', body }
        )
            .then(async (res) => {
                ...
                if (!json || json['error']) {
                    toast({
                        title: 'Failed to update invites punishment',
                        description:
                            json?.error ??
                            'An error occurred while updating invites punishment.',
                        status: 'error',
                    });
                    return;
                }
                toast({
                    title: 'Updated Invites Punishment',
                    description: `Users will now receive a ${PunishmentNames[data.punishment ?? 'WARN'].name} after breaking the invites limit.`,
                    status: 'success',
                });
                ...
                reset({
                    punishment: json.automod_invites_punishment.punishment,
                    reason: json.automod_invites_punishment.reason,
                });
                setSuccess(true);
            })
            .catch(() => {});
