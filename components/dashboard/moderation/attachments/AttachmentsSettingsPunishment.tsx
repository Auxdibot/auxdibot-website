...
type AttachmentsLimitPunishmentBody = {
    punishment: PunishmentType | null;
    reason?: string;
};

export default function AttachmentsSettingsPunishment({
    server,
}: {
    server: {
    ...
    const { control, reset, handleSubmit } =
        useForm<AttachmentsLimitPunishmentBody>({
            defaultValues: useMemo(
                () => ({
                    punishment:
                        server.automod_attachments_punishment?.punishment ??
                        'WARN',
                    reason:
                        server.automod_attachments_punishment?.reason ??
                        'You have been punished for breaking the attachments limit on this server.',
                }),
    ...
    function onSubmit(data: AttachmentsLimitPunishmentBody) {
        ...
        fetch(
            `/bot/v1/servers/${server.serverID}/moderation/attachments/punishment`,
            { method: 'POST', body }
        )
            .then(async (res) => {
                ...
                if (!json || json['error']) {
                    toast({
                        title: 'Failed to update attachments punishment',
                        description:
                            json?.error ??
                            'An error occurred while updating attachments punishment.',
                        status: 'error',
                    });
                    return;
                }
                toast({
                    title: 'Updated Attachments Punishment',
                    description: `Users will now receive a ${PunishmentNames[data.punishment ?? 'WARN'].name} after breaking the attachments limit.`,
                    status: 'success',
                });
                setSuccess(true);
                ...
                reset({
                    punishment: json.automod_attachments_punishment.punishment,
                    reason: json.automod_attachments_punishment.reason,
                });
            })
            .catch(() => {});
    }
