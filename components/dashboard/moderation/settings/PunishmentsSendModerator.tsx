...
export default function PunishmentsSendModerator({ server }: { server: { serverID: string, punishment_send_moderator: boolean } }) {
    ...
    function handleClick() {
        fetch(`/api/v1/servers/${server.serverID}/moderation/send_moderator`, { method: "POST" }).then(async (data) => 
        {
            ...
            ? actionContext.setAction({ status: json['data']?.punishment_send_moderator ? 
            'Auxdibot will send the punishment moderator when notifying a member of their punishment.' : 
            'Auxdibot will no longer send the punishment moderator when notifying a member of their punishment', success: true }) : ""
        }).catch(() => undefined);
    }
