...
export const columns: (serverID: string) => ColumnDef<ScheduleType>[] = (serverID) => [
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return row.original.times_run.toString() + "/" + (row.original.times_to_run ?? "∞");
      },
      header: ({ column }) => (
      ...
      cell({ row }) {
        ...
        function deleteSchedule() {
          fetch(`/api/v1/servers/${serverID}/schedules/${row.original.index}`, { method: "DELETE" }).then(async (data) => {
              ...
              toast({ title: "Schedule Deleted", description: `Schedule #${row.original.index+1} was deleted.`, status: "success" })
          })
          }
