...
export default function LevelEventXP({ server }: { server: LevelPayload }) {
    const [eventXP, setEventXP] = useState<(number | string)[]>([
        server.event_xp_range[0] ?? 0,
        server.event_xp_range[1] ?? 0,
    ]);
    ...
    function setEventXPGiven() {
        ...
        fetch(`/bot/v1/servers/${server.serverID}/levels/event_xp`, {
            method: 'POST',
            body,
        })
            .then(async (data) => {
                const json:
                    | { data: { event_xp_range: number[] } }
                    | { error: string } = await data
                    .json()
                    .catch(() => undefined);
                if (!json || 'error' in json) {
                    toast({
                        title: 'Failed to update event XP',
                ...
                toast({
                    ...
                    description: `Successfully updated event XP to be ${!json.data.event_xp_range[1] ? json.data.event_xp_range?.join(' to ') : json.data.event_xp_range[0]} XP.`,
                    status: 'success',
                });
        ...
        <div className={'mx-auto flex w-fit flex-col gap-1 p-4'}>
            ...
            >
                ...
                >
                    ...
                    >
                        <Input
                            value={Number(eventXP[0]) || 0}
                            className={'rounded-r-none border-0'}
                            onChange={(e) =>
                                onEventXPChange([e.target.value, eventXP[1]])
                            }
                        />
                    </span>
                    <span
                    ...
                    <span
                        className={
                            'flex w-24 rounded-r-xl border border-gray-700 bg-gray-950'
                        }
                    >
                        <Input
                            value={Number(eventXP[1]) || 0}
                            className={'rounded-none border-0'}
                            onChange={(e) =>
                                onEventXPChange([eventXP[0], e.target.value])
                            }
                        />
                    </span>
                </div>
                <Button
                    onClick={() => setEventXPGiven()}
