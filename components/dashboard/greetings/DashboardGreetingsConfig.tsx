...
import { BsChatLeftDots, BsImage, BsListTask, BsPencil, BsPerson, BsPlus, BsTextCenter, BsTextLeft, BsTextarea, BsX } from 'react-icons/bs';
...
export default function DashboardGreetingsConfig({ id }: { id: string }) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className={"flex flex-col gap-2 md:m-5 my-5"}>
        <label className={"flex flex-row max-xl:flex-col gap-2 items-center font-lato text-xl"}>
            <span className={"flex flex-row gap-2 items-center"}><PiHandWavingLight/> Greeting:</span> 
            <select className={"rounded-md font-roboto w-fit text-lg"} {...register("greeting", { required: true })}>
            <option value={GreetingType.JOIN}>Join</option>
            <option value={GreetingType.JOIN_DM}>Join DM</option> 
            <option value={GreetingType.LEAVE}>Leave</option> 
        </select></label>
    ...
    <div className={"bg-gray-800 shadow-2xl border-2 border-gray-800 rounded-2xl max-md:h-fit w-full max-md:mx-auto h-full"}>
