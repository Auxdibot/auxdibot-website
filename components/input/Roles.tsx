...
export default function Roles({ serverID, onChange, value, required }: RolesInputProps) {
    ...
    return (<span className={"relative flex items-center"} ref={inputRef}>
            <span onClick={() => setCollapsed(!collapsed)}  style={{ backgroundColor: roleValue?.color ? '#' + roleValue?.color.toString(16) : '' }} className={"flex items-center gap-1 group cursor-pointer bg-gray-700 p-1 rounded-lg font-open-sans"}>{value ? <BsAt/> : required ? '' : <BsX/>} {value ? roleValue?.name :  required ? 'Select a role...' : 'No Role'} <span>
                <BsArrowDownShort className={"transition-all group-hover:translate-y-1"}/></span>
                </span>
            <div className={`absolute overflow-hidden border border-gray-500 transition-all shadow-xl bg-gray-700 rounded-lg w-max top-full translate-y-1 origin-top-left max-md:origin-top max-md:-translate-x-1/2 left-1/2 ${collapsed ? 'scale-0' : 'scale-100'}`}>
            ...
            {roles?.map((i: { id: string, name: string, color: number }) => i.name != '@everyone' ? <a style={{ backgroundColor: i.color ? '#' + i.color.toString(16) : '' }} className={"flex items-center w-full gap-1 cursor-pointer hover:gap-2 transition-all p-0.5 rounded-lg bg-gray-800"} key={i.id} onClick={() => change(i.id)}><BsAt/> {i.name}</a> : "")}
            </div>
            </div>
