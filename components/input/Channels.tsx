...
export default function Channels({ serverID, onChange, value, required }: ChannelsInputProps) {
    ...
    return (<span className={"relative flex items-center"} ref={inputRef}>
            ...
            <div className={`absolute z-10 overflow-hidden border border-gray-500 transition-all shadow-xl bg-gray-700 rounded-lg w-max top-full translate-y-1 origin-top-left max-md:origin-top max-md:-translate-x-1/2 max-md:left-1/2 ${collapsed ? 'scale-0' : 'scale-100'}`}>
