...
export default function LayoutNavbar({ preventCollapse, premiumIcon, serverID }: { preventCollapse?: boolean, premiumIcon?: boolean, serverID?: string }) {
    ...
    <nav className={`${preventCollapse ? "fixed bg-gradient-to-br from-gray-900 to-black border-b border-slate-800" : "absolute"} transition-all flex align-middle items-center gap-10 max-md:gap-4 w-full px-5 py-2 h-16 z-50`}>
        ...
        <div className={"flex gap-5 max-md:ml-2 items-center font-montserrat text-gray-100"}> 
            ...
            <Link href={process.env.NEXT_PUBLIC_DISCORD_SERVER_LINK || ""} target="_blank" className={"flex gap-2 items-center max-sm:text-md text-xl group relative before:hover-underline before:primary-gradient hover:before:scale-100"}><BsDiscord className={"group-hover:scale-110 group-hover:text-orange-500 origin-center transition-all"}/> <span className={"max-md:hidden tracking-wide"}>support</span></Link>
            <Link href={process.env.NEXT_PUBLIC_DISCORD_INVITE_LINK || ""} target="_blank" className={"flex gap-2 items-center max-sm:text-md text-xl group relative before:hover-underline before:primary-gradient hover:before:scale-100"}><BsArrowUp className={"group-hover:scale-110 group-hover:text-orange-500 origin-center transition-all"}/> <span className={"max-md:hidden tracking-wide"}>invite</span></Link>
