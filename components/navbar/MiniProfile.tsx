...
export default function MiniProfile(props: React.ComponentProps<any>) {
    ...
    return (<div ref={ref} {...props}>
        ...
        <span className={"flex flex-row mx-auto text-base gap-2 bg-gray-900 py-1 px-2 justify-center rounded-2xl w-fit items-center"}>{user?.badges?.map((i: UserBadge) => <span key={i}>{UserBadgeIcons[i]}</span>)}</span>
        </span>
