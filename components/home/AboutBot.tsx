...
export default function AboutBot() {
  const [isServer, setIsServer] = useState(true)
    ...
    const variants = useMemo(() => mobile && !isServer
    ? { transform: "translateY(-4rem)", opacity: 0 }
    : { transform: "translateX(-4rem)", opacity: 0 }, [mobile, isServer]);
    const canvasVariants = useMemo(() => mobile && !isServer
      ? { transform: "translateY(-4rem)", opacity: 0 }
      : { transform: "translateX(4rem)", opacity: 0 }, [mobile, isServer]);
      useEffect(() => {
        if (isServer) {
    ...
    return (
        ...
        <motion.h1 initial={variants} viewport={{ once: true }} whileInView={{ transform: "translateX(0px)", opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }} className={"text-6xl text-left max-lg:text-center max-lg:mx-auto  max-lg:items-center flex flex-col justify-center lowercase w-full header"}>About Auxdibot</motion.h1>
        <motion.p initial={variants} viewport={{ once: true }} whileInView={{ transform: "translateX(0px) translateY(0px)", opacity: 1 }} transition={{ duration: 0.5, delay: 0.6 }} className={'font-inter text-zinc-200 custom-underline text-xl max-lg:text-center'}>Auxdibot is a Discord bot project founded and maintained by <Link href={"https://auxdible.me"} className={"relative before:hover-underline before:bg-gradient-to-br before:from-primary-100 before:to-primary-600 hover:before:scale-100 font-raleway text-xl font-bold"}>Auxdible</Link>. Auxdibot features a wide variety of features for admins to manage their servers with. Auxdibot receives consistent updates and constant bug fixes, making it a reliable choice for your server!</motion.p>

        <span className='flex gap-5 max-lg:justify-center max-[320px]:flex-col items-center'>
            <motion.span initial={{ transform: "translateY(2rem)", opacity: 0 }} viewport={{ once: true }} whileInView={{ transform: "translateY(0px)", opacity: 1 }} transition={{ duration: 0.5, delay: 1 }}><HeaderButton className='flex w-fit text-xl font-bold' href={process.env.NEXT_PUBLIC_DOCUMENTATION_LINK}><span className='flex items-center gap-2'><Book/> Documentation</span></HeaderButton></motion.span>
        ...
        <motion.div initial={canvasVariants} viewport={{ once: true }} whileInView={{ transform: "translateX(0px) translateY(0px)", opacity: 1 }} transition={{ duration: 0.5 }}  className="max-lg:w-[250px] max-w-xl lg:flex-1 lg:self-stretch max-lg:h-[200px] touch-none">
        ...
        <Icon frustumCulled={false} scale={[26.5,26.5,26.5]} rotation={[0,0,0]} frontToBack/>
