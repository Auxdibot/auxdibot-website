...
export default function AboutBot() {
    ...
    return (
        ...
        <section className={"mx-auto max-w-7xl w-full flex justify-center items-center max-md:flex-col gap-10 min-h-[400px] max-lg:overflow-x-hidden"}>
        <div className='flex-1 max-w-2xl'>
        <div className='max-xl flex flex-col gap-4'>
        <motion.h1 initial={{ transform: "translateX(-4rem)", opacity: 0 }} viewport={{ once: true }} whileInView={{ transform: "translateX(0px)", opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }} className={"text-6xl text-left max-md:text-center max-md:mx-auto  max-md:items-center flex flex-col justify-center lowercase w-full header"}>About Auxdibot</motion.h1>
        <motion.p initial={{ transform: "translateX(-4rem)", opacity: 0 }} viewport={{ once: true }} whileInView={{ transform: "translateX(0px)", opacity: 1 }} transition={{ duration: 0.5, delay: 0.6 }} className={'font-inter text-zinc-200 custom-underline text-xl max-md:text-center'}>Auxdibot is a Discord bot project founded and maintained by <Link href={"https://auxdible.me"} className={"relative before:hover-underline before:bg-gradient-to-br before:from-primary-100 before:to-primary-600 hover:before:scale-100 font-raleway text-xl font-bold"}>Auxdible</Link>. Auxdibot features a wide variety of features for admins to manage their servers with. Auxdibot receives consistent updates and constant bug fixes, making it a reliable choice for your server!</motion.p>
        ...
        <motion.div initial={{ transform: "translateX(4rem)", opacity: 0 }} viewport={{ once: true }} whileInView={{ transform: "translateX(0px)", opacity: 1 }} transition={{ duration: 0.5 }}  className="max-md:w-[250px] max-w-xl md:flex-1 md:self-stretch max-md:h-[200px] touch-none">
