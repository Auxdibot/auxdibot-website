...
export default function About() {
    ...
    return (
        ...
        <motion.h1 initial={{ transform: "translateX(4rem)", opacity: 0 }} viewport={{ once: true }} whileInView={{ transform: "translateY(0px)", opacity: 1 }} transition={{ duration: 0.5, delay: 0.5 }} className={"text-6xl text-end max-md:text-center max-md:mx-auto items-end max-md:items-center flex flex-col justify-center lowercase"}><span className={"header"}>About</span><span className={"header"}>Auxdibot</span></motion.h1>
        <motion.span initial={{ scaleY: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }} whileInView={{ scaleY: 1 }} className={"flex-grow w-1 border-r-2 border-gray-400 max-md:hidden"}></motion.span>
        <motion.p initial={{ transform: "translateX(-4rem)", opacity: 0 }} viewport={{ once: true }} whileInView={{ transform: "translateX(0px)", opacity: 1 }} transition={{ duration: 0.5, delay: 1.0 }} className={'font-open-sans text-gray-400 text-2xl max-md:text-center'}>Auxdibot is a Discord bot project founded and maintained by <Link href={"https://auxdible.me"} className={"header font-montserrat font-bold"}>Auxdible</Link>. Auxdibot features a wide variety of features for admins to manage their servers with. Auxdibot receives consistant updates and constant bug fixes, making it a reliable choice for your server!</motion.p>
        </section>
