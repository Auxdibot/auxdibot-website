...
export default function EmojiPicker({
        ...
        const paste = (e: globalThis.ClipboardEvent) => {
            ...
            if (
                ...
                emojis.find((i) =>
                    i.emojis.find(
                        (emoji) =>
                            emoji.emoji.codePointAt(0) === text.codePointAt(0)
                    )
                )
            ) {
                e.preventDefault();
    ...
    const emojiValue = value
        ? emojis
              .find((i) => i.emojis.find((emoji) => emoji.emoji == value))
              ?.emojis.find((emoji) => emoji.emoji == value)
        : undefined;
    return (
        <span className={'relative flex items-center'} ref={inputRef}>
            {isDesktop ? (
                <Popover modal onOpenChange={(open) => setOpen(open)}>
                    <PopoverTrigger asChild>
                        <Button variant={'outline'} className={className}>
                            {value ? (
                                ...
                                <span
                                    className={
                                        'flex items-center gap-2 text-sm italic text-gray-400'
                                    }
                                >
                                    <Twemoji>👋</Twemoji> Select an Emoji
                                </span>
                            )}
                        </Button>
                    </PopoverTrigger>
