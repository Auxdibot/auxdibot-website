...
function TextareaMessage({
    ...
    const { data: placeholdersRes } = useQuery<{
        ...
        ['placeholders', serverID],
        async () =>
            await fetch(
                `/bot/v1/placeholders?${Array.isArray(placeholderContext) ? placeholderContext.map((i) => `context=${i}`, '').join('&') : `context=${placeholderContext ?? 'null'}`}`
            )
                .then(async (data) => await data.json().catch(() => undefined))
                .catch(() => undefined)
    );
    const { data: serverEmojis } = useQuery<ServerEmojiBody | undefined>(
    ...
    const channels =
        !channelsRes || channelsRes['error'] ? [] : sortChannels(channelsRes);
    const placeholders = placeholdersRes?.placeholders
        ? Object.keys(placeholdersRes.placeholders)
        : [];
    ...
    return (
        <span
            className={cn('relative min-h-[100px] max-md:px-2', wrapperClass)}
        >
            <div
                                ...
                                {isDesktop ? (
                                    ...
                                    <Drawer>
                                        <DrawerTrigger asChild>
                                            <Button
                                                className={
                                                    'w-full justify-start gap-1 p-0 pl-2'
                                                }
                                                variant={'link'}
                                            >
                                                <BsHash /> Channels
            ...
            {props.maxLength ? (
                <span className={'my-2 pl-2 text-sm text-gray-400'}>
